...
# 定义目标文件夹路径
folder_paths = [
    r"E:\01我的\大三下(202501-202508)\大创-压电阻抗\14天阻抗汇总\Term1",
    r"E:\01我的\大三下(202501-202508)\大创-压电阻抗\14天阻抗汇总\Term2"
]
...
for folder_path in folder_paths:
    for i in range(1, 31):  # 30 个文件
        filename = f"1-500 12.24_{i:02d}.xlsx" if "Term1" in folder_path else f"1-500 01.17_{i:02d}.xlsx"
        file_path = os.path.join(folder_path, filename)

        # 读取Excel的B列数据（B2-B2497对应索引1-2496）
        df = pd.read_excel(file_path, usecols="B", header=None, skiprows=1, nrows=2496)
        # 转换为向量并添加到矩阵
        Imp1_500_Orgl.append(df.iloc[:, 0].values)

# 转换为 numpy 数组（60行×2496列）
Imp1_500_Orgl = np.array(Imp1_500_Orgl)
...
# 法2：求和削峰
divider = 2 # 相邻（）个一求和
Imp1_500 = Imp1_500_Orgl.reshape(60, int(2496 / divider), divider).sum(axis=2)
