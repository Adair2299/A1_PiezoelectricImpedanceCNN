...
def residual_block_with_attention(x, filters, kernel_size, pooling=True):
    x, shortcut = residual_block(x, filters, kernel_size, pooling=False)
    # 注意力在卷积后
...
def build_advanced_model(input_length):
    ...
    # 第一个残差块（无注意力）
    x, shortcut = residual_block(x, 128, 3)
    x = Add()([shortcut, x])
    x = LayerNormalization()(x)
    x = MaxPooling1D(pool_size=2)(x)

    # 第二个残差块（带注意力）
    x = residual_block_with_attention(x, 128, 3, pooling=True)
