...
# Dual-Attention Network模型
def dual_attention_model(input_shape):
    inputs = Input(shape=input_shape)

    # 时间注意力
    time_attention = MultiHeadAttention(num_heads=8, key_dim=128)(inputs, inputs)  # 增加头数和维度
    time_attention = LayerNormalization()(time_attention)

    # 空间注意力
    space_attention = MultiHeadAttention(num_heads=8, key_dim=128)(time_attention, time_attention)  # 同上
    space_attention = LayerNormalization()(space_attention)

    x = Dense(128, activation='relu')(space_attention)  # 增加神经元数
    x = Flatten()(x)
...
model = dual_attention_model((sequence_length, 1))
model.summary()
