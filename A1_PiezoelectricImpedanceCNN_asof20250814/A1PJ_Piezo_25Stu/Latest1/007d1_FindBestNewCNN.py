...
# 构建模型
def build_advanced_model(input_length, filters, kernel_size, dropout_rate, n_residual_blocks, initial_pool_size):
    ...
    for i in range(n_residual_blocks):
        pooling = i < n_residual_blocks - 1  # Only pool in all but last block
        x = residual_block(x, filters, kernel_size, pooling=pooling, pool_size=2)  # Fixed pool_size=2 in residual blocks
...
# 目标函数，传入超参数进行训练
def objective(trial):
    filters = 64 # trial.suggest_categorical('filters', [32, 64, 128])
    kernel_size = trial.suggest_int('kernel_size', 5, 12)
    learning_rate = 1e-3 # trial.suggest_loguniform('learning_rate', 1e-5, 1e-2)
    dropout_rate = 0.3 # trial.suggest_uniform('dropout_rate', 0.2, 0.5)
    n_residual_blocks = trial.suggest_int('n_residual_blocks', 1, 5)  # 1-3 residual blocks
    initial_pool_size = trial.suggest_int('initial_pool_size', 2, 8)  # Initial pooling size 4-8

    # 构建模型
    model = build_advanced_model(Imp1_500.shape[1], filters, kernel_size, dropout_rate,
    ...
    print(f"Validation MAE for trial: {val_mae:.4f}")
...
best_results.to_csv("best_hyperparameters.csv", index=False)
