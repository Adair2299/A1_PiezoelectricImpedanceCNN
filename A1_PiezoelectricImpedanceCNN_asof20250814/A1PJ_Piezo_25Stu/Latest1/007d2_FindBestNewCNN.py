...
def objective(trial):
    ...
    n_residual_blocks = trial.suggest_int('n_residual_blocks', 1, 5)  # 限制最大块数
    initial_pool_size = trial.suggest_int('initial_pool_size', 2, 8)
    residual_pool_size = trial.suggest_int('residual_pool_size', 2, 4)  # Pool size within residual blocks
    use_ln = trial.suggest_categorical('use_layer_norm', [True, False])  # Whether to use LayerNorm
    use_dropout = trial.suggest_categorical('use_dropout', [True, False])  # Whether to use Dropout in res blocks
...
study.optimize(objective, n_trials=30, show_progress_bar=True)  # Increased trials to 30 for better exploration
