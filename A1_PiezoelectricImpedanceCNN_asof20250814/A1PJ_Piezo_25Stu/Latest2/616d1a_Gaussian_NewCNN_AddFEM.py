# 最新加有限元+打印，但是没有去除腐蚀率为0的，复盘请用115d1a
# 去除质量不好的EMI
# loss: mse和Huber的比较
...
result = pd.read_excel(
    r"E:\01我的\大三下(202501-202508)\大创-压电阻抗\数据分析\数据操作\11_masslossrateAddFEM2.xlsx",
    header=None,          # 无列名
    usecols="B",          # 只读取B列
    nrows=207,           # 读取207行
    skiprows=0           # 从第1行开始读（B1）
).values                 # 转换为NumPy矩阵
...
# 文件路径
file_path = Path(r"E:\01我的\大三下(202501-202508)\大创-压电阻抗\14天阻抗汇总\Term all 1-500 AddFEM2.xlsx")

# 精确读取 B2:DM2497
try:
    df = pd.read_excel(file_path, usecols="B:GZ", header=None, skiprows=1, nrows=2496)
...
X_val, X_test = X_remaining[:int(len(X_remaining) // 1.05)], X_remaining[int(len(X_remaining) // 1.05):] # 15% 验证, 15% 测试
y_val, y_test = y_remaining[:int(len(y_remaining) // 1.05)], y_remaining[int(len(y_remaining) // 1.05):]
val_indices = remaining_indices[:int(len(X_remaining) // 1.05)]
test_indices = remaining_indices[int(len(X_remaining) // 1.05):]
...
model.compile(optimizer=optimizer, loss=Huber(delta=0.02), metrics=['mae'])  # 增加MAE监控 Huber(delta=2e-3)
model.summary()
...
# test_relative_errors[test_relative_errors > 0.8] = np.nan
...
model_folder_name = f"完整模型AddFEM-Huber(delta=0.02)-{current_time}"
performance_report = f'''
=== Performance Report By 616d1a ===
...
output_folder = Path(r"E:\01我的\大三下(202501-202508)\大创-压电阻抗\论文材料\AddFEM") / model_folder_name
...
plt.savefig(output_folder / "training_process.png", dpi=600, bbox_inches='tight') # 保存专用
plt.close()  # 保存专用 # 关闭图形，避免重叠
...
# --------------------- 子图1：真实值 vs 预测值 --------------------
sample_numbers = np.arange(1, len(y_test)+1)  # 横坐标从1开始计数
...
# 设置横坐标刻度（每5个样本一个刻度）
xticks = np.arange(1, len(y_test)+1, 5)  # 从1开始，步长=5
ax1.set_xticks(xticks)
ax1.set_xticklabels(xticks)  # 确保标签为整数
...
plt.tight_layout() # 保存专用
plt.savefig(output_folder / "prediction_comparison.png", dpi=600, bbox_inches='tight') # 保存专用
plt.close()  # 保存专用 # 关闭图形

# 3. 保存性能报告为图片
fig = plt.figure(figsize=(8, 6)) # 保存专用
gs = GridSpec(1, 1, figure=fig) # 保存专用
ax = fig.add_subplot(gs[0, 0]) # 保存专用
ax.axis('off') # 保存专用
ax.text(0.1, 0.9, 'Model Performance Report', fontsize=14, fontweight='bold') # 保存专用
ax.text(0.1, 0.8, performance_report, fontsize=10, va='top', ha='left') # 保存专用


# ---------------------4. 添加相对误差分布图 --------------------
plt.figure(figsize=(6, 4))
plt.hist(test_relative_errors, bins=20, color='skyblue', edgecolor='black', alpha=0.7, linewidth=0.5)
plt.title('Test Set Relative Error Distribution', fontweight='bold')
plt.xlabel('Relative Error (%)')
plt.ylabel('Probability Distribution (%)')
plt.grid(True, linestyle='--', alpha=0.5)
plt.savefig(output_folder / "relative_error_distribution.png", dpi=600, bbox_inches='tight')
plt.close()

plt.tight_layout()
plt.savefig(output_folder / "performance_report.png", dpi=600, bbox_inches='tight') # 保存专用
plt.close()
