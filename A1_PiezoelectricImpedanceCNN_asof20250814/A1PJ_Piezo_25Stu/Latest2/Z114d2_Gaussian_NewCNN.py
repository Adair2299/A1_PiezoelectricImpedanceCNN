...
def residual_block(x, filters_1=64, filters_2=32, kernel_size=3, pooling=True):
    shortcut = x

    # 第一层卷积: 64 filters, kernel_size=3
    x = Conv1D(filters_1, kernel_size, padding='same', activation='relu')(x)
    x = LayerNormalization()(x)
    x = Dropout(0.2)(x)

    # 第二层卷积: 32 filters, kernel_size=3
    x = Conv1D(filters_2, kernel_size, padding='same')(x)

    # 调整shortcut的维度以匹配第二层卷积的输出 (32 filters)
    if shortcut.shape[-1] != filters_2:
        shortcut = Conv1D(filters_2, 1, padding='same')(shortcut)  # 用1x1卷积调整通道数
...
def build_advanced_model(input_length):
    ...
    # 修改后的残差块
    x = residual_block(x, filters_1=64, filters_2=32, pooling=True)

    # 注意：后续注意力机制的输入通道数需匹配残差块输出（32）
    x = ChannelAttention(x)
    x = TemporalAttention(x)
...
lr_scheduler = ReduceLROnPlateau(
    monitor='val_loss', factor=0.8, patience=64, min_lr=5e-4, verbose=1
)
