...
lr_scheduler = ReduceLROnPlateau(
    monitor='val_loss', factor=0.9, patience=96, min_lr=6.5e-4, verbose=1
)
...
model.compile(optimizer=optimizer, loss=Huber(delta=0.1), metrics=['mae'])  # 增加MAE监控 Huber(delta=2e-3)
model.summary()
...
# 打印表格
print("\n=== 模型性能指标 ===")
print(f"{'':<15} {'R²':<8} {'RMSE':<8} {'MAE':<8}")
print(f"{'Training Set':<15} {train_metrics['R²']:.3f}   {train_metrics['RMSE']:.4f}   {train_metrics['MAE']:.4f}")
print(f"{'Validation Set':<15} {val_metrics['R²']:.3f}   {val_metrics['RMSE']:.4f}   {val_metrics['MAE']:.4f}")
print(f"{'Test Set':<15} {test_metrics['R²']:.3f}   {test_metrics['RMSE']:.4f}   {test_metrics['MAE']:.4f}")
...
model_folder_name = f"完整模型Huber(delta=0.1)-{current_time}-Disprove Overfitting"
performance_report = f'''
=== Performance Report By 115d1a === [Epoch 2000]
...
plt.plot(history.history['lr'], 'g-', linewidth=1)
plt.title('Learning Rate Schedule')
plt.xlabel('Epoch')
plt.ylabel('Learning Rate')
plt.grid(True)
plt.savefig(output_folder / "learning_rate_curve.png", dpi=600, bbox_inches='tight')
plt.close()
